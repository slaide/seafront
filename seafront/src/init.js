...
/** from p2.js */
let _p = new Manager()
window.addEventListener("load", () => _p.start())
...
function updateMicroscopePosition() {
    ...
    async function onload(data) {
        ...
        if (data.latest_imgs != null && channel_names.length > 0) {
            ...
            try { get_histogram_plot_element() } catch (e) { console.error("histogram_plot_element not present"); return }
            get_histogram_plot_element().data.forEach((trace, index) => {
            ...
            if (indicesToRemove.length > 0) {
                Plotly.deleteTraces(get_histogram_plot_element(), indicesToRemove);
            }
                ...
                if ((last_channel_update_timestamps.get(channel_data.channel.handle) || 0) < channel_timestamp) {
                    ...
                    channel_updater.schedule(channel_data).then(async data => {
                        ...
                        if (!(latest_channel_image_canvas instanceof HTMLCanvasElement)) {
                            ...
                            Plotly.addTraces(get_histogram_plot_element(), {
                                x: histogram_data.map((val, i, ar) => i),
                                y: histogram_data,
                                type: "scatter",
                                mode: "lines",
...
console.log("defined microscope config")
...
function microscopeConfigOverride(new_config) {
    ...
    interval_id = setInterval(function () {
        ...
        const currentPlateType = microscope_config.wellplate_type
        ...
        function getIndexFromRowCol(row, col) {
            ...
            return row * (WellplateType.num_wells(currentPlateType) + 3) + col
        }
...
class WellplateType {
     ...
     */
    static num_wells(pt) {
        return pt.Num_wells_x * pt.Num_wells_y
    }

    /** get all plate specs from the server, and cache the result */
    static all_raw_plates = (get_hardware_capabilities()?.wellplate_types || [])

    /** @type {{name:string,num_wells:number,entries:Wellplate[]}[]?} */
    static _all = null

    /** @type {Map<string,Wellplate>} */
    ...
    static get all() {
        ...
        for (let new_plate_type of plate_types) {
            ...
            let entries = null
            for (let e of ret) {
                if (e.num_wells == WellplateType.num_wells(new_plate_type)) {
                    entries = e.entries
                    break
                }
            }
            if (entries == null) {
                entries = []
                ret.push({
                    name: WellplateType.num_wells(new_plate_type) + " well plates",
                    num_wells: WellplateType.num_wells(new_plate_type),
                    entries: entries
                })
            }
    ...
    static fromHandle(handle) {
        return WellplateType._handleToType.get(handle) ?? null
    }
}

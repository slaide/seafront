...
function initwellnavigator() {
    ...
    const plate_type = microscope_config.wellplate_type

    // TODO this is really not smart. the Model_id by itself (!) is bound to the interface, but
    // the rest of the wellplate type specification is out of sync in between interface interaction
    // and this line being run. works fine in practice, but is really bad style.
    Object.assign(microscope_config.wellplate_type, WellplateType.fromHandle(microscope_config.wellplate_type.Model_id))
    ...
    let plate_type_font_size = "--plate-" + WellplateType.num_wells(plate_type) + "-font-size"
    ...
    if (forbidden_wells_str) {
        let forbidden_wells_dict = JSON.parse(forbidden_wells_str.value.toString())

        for (let numwells of Object.keys(forbidden_wells_dict)) {
            if (parseInt(numwells) != WellplateType.num_wells(plate_type)) continue
            /** @type {string[]} */
            let forbidden_wells_names_list = forbidden_wells_dict[numwells]

            forbidden_wells_names_list.forEach(w_str => plate_type_forbidden_wells.add(w_str))
        }
    }
    // console.log(`forbidden wells for plate type:`, plate_type, plate_type_forbidden_wells)

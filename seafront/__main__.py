...
from ome_types._autogenerated.ome_2016_06.xml_annotation import Value
...
class Core:
    ...
    def __init__(self, selected_microscope: CriticalMachineConfig):
        ...
        route_wrapper(
            "/api/action/execute_commands",
            CustomRoute(handler=self.execute_commands, tags=[RouteTag.ACTIONS.value]),
            methods=["POST"],
        )
    ...
    async def execute_commands(self, commandlist: str) -> BasicSuccessResponse:
        """
        Execute a list of commands from a newline-separated string.

        uses gcode format
        https://reprap.org/wiki/G-code
        """
        lines = commandlist.split('\n')
        ...
        for linenum,line in enumerate(lines):
            line_and_comment=line.split(";",maxsplit=1)
            # comment is optional
            ...
            # split by segments
            segments=[s for s in line.split(" ") if len(s)>0]

            assert len(segments)>0

            commandname=segments[0]

            match commandname:
                case "G0"|"G1": # move (rapid/linear, which we treat the same)
                    command=cmd.MoveTo()
                    ...
                    for segment in segments[1:]:
                        match segment[0]:
                            case "X":
                                target_x_mm=float(segment[1:])
                                if np.isnan(target_x_mm):
                                    continue

                                if state.relative_mode:
                                    command.x_mm=target_x_mm + state.zero_x_mm + current.adapter_state.stage_position.x_pos_mm
                                else:
                                    command.x_mm=target_x_mm + state.zero_x_mm
                            case "Y":
                                target_y_mm=float(segment[1:])
                                if np.isnan(target_y_mm):
                                    continue

                                if state.relative_mode:
                                    command.y_mm=target_y_mm + state.zero_y_mm + current.adapter_state.stage_position.y_pos_mm
                                else:
                                    command.y_mm=target_y_mm + state.zero_y_mm
                            case "Z":
                                target_z_mm=float(segment[1:])
                                if np.isnan(target_z_mm):
                                    continue

                                if state.relative_mode:
                                    command.z_mm=target_z_mm + state.zero_z_mm + current.adapter_state.stage_position.z_pos_mm
                                else:
                                    command.z_mm=target_z_mm + state.zero_z_mm
                            
                            case other:
                                error_internal(f"line {linenum}: command {commandname} invalid argument {other}")

                    await self.microscope.execute(command)
                ...
                case "G4": # dwell (wait)
                    for segment in segments[1:]:
                        ...
                        match segment[0]:
                            case "P":
                                target_time_ms=float(segment[1:])
                                if np.isnan(target_time_ms):
                                    continue

                                time_s=target_time_ms*1e-3
                            case "S":
                                target_time_s=float(segment[1:])
                                if np.isnan(target_time_s):
                                    continue
                ...
                case "G20": # set units to inches
                    error_internal("inches not allowed!!!")
                case "G21": # set units to mm (default)
                    pass # already

                case "G28": # home (the link above has great info on this)
                    command=cmd.Home()

                    # if no arguments given, default to homing all axes
                    if len(segments)==1:
                        command.x=True
                        command.y=True
                        command.z=True
                ...
                case "G90": # set to absolute positioning (relative to machine zero, kinda) \
                            # (can be somewhat overriden by G92)
                    state.relative_mode=False
                case "G91": # set to relative positioning (relative to last position)
                ...
                case "G92": # set position
                    ...
                    for segment in segments[1:]:
                        match segment[0]:
                            case "X":
                                newxvalue=float(segment[1:])
                                if np.isnan(newxvalue):
                                    continue

                                state.zero_x_mm=current.adapter_state.stage_position.x_pos_mm - newxvalue
                            case "Y":
                                newyvalue=float(segment[1:])
                                if np.isnan(newyvalue):
                                    continue

                                state.zero_y_mm=current.adapter_state.stage_position.y_pos_mm - newyvalue
                            case "Z":
                                newzvalue=float(segment[1:])
                                if np.isnan(newzvalue):
                                    continue

                                state.zero_z_mm=current.adapter_state.stage_position.z_pos_mm - newzvalue
                            case other:
                                error_internal(f"line {linenum}: command {commandname} invalid argument {other}")

                case "M0": # [unconditional] stop
                    pass # halt machine (which we cannot do well...)
                
                case "M1": # sleep or unconditional stop
                    pass

                case "M2": # program end
                    break # stop program execution and return

                case "M16": # expected printer check
                    pass # example: M16 squid 187-2vd <- would then compare the current printer name \
                         # to the string "squid 187-2vd" and halt if it does not match

                case "M17": # enable all stepper motors
                    pass

                case "M18": # disable all stepper motors
                    pass

                # there are a bunch of scard commands, e.g. M32 select file and start sd print.. seems like a way
                # for us to start a program from a file as well, including subroutines like homing..
                # there is also M20 to list all files on the sdcard
                # M470+ for sd file operations

                case "M37": # simulation mode control
                    error_internal("explicitely enabling/disabling simulation mode is not currently supported")

                case "M42": # switch io pin
                    pass # .. use this to control lights..?

                case "M46": # show assigned IP
                    pass

                case "M98": # execute macro/subprogram
                    error_internal("subprogram execution not currently supported")
                case "M99": # return from macro/subprogram
                    error_internal("subprogram execution not currently supported")

                case "M112": # immediate/emergency stop
                    break # just stop loop and return

                case "M115": # get firmware version and capabilities (!!!!!)
                    error_internal("m115 not yet implemented")

                case "M116": # wait [for slow changing variables to arrive]
                    pass

                case "M125": # park head
                             # save current position then move to park position..
                             # maybe useful for loading position
                    pass

                case "M124": # set cooler temperature
                    pass # maybe for camera temperature control

                case "M141": # set chamber temperature
                    pass # for incubator controls

                case "M143": # set laser cooler temperature
                    pass # .. for laser temperature control

                case "M146": # set chamber humidity
                    pass # for incubator controls

                case "M154": # auto report position
                    pass # essentially toggle between push and pull modes

                case "M191": # wait for chamber temperature to reach target temp
                    pass

                case "M201": # set max acceleration
                    pass
                case "M201.1": # set max acceleration for special mvoes
                    pass
                case "M202": # set max acceleration for travel moves
                    pass
                case "M204": # set default acceleration
                    pass # for print and travel

                case "M205": # advanced settings
                    pass # e.g. max per axis jerk

                case "M211": # enable/disable software endstops (only in a single direction..)
                    pass

                case "M218": # set hotend offset
                    pass # could be repurposed to set channel parameters

                case "M240": # trigger camera
                    pass # !!!!

                case "M260": # i2c send data
                    pass # seems useful... (for new hardware) m261 request i2c datas

                case "M306": # set home offset calculated from toolhead position
                    pass # similar to special case use of G92 (to calibrate stage position)

                case "M350": # set microstepping mode
                    pass # arg is 1/n microsteps

                case "M360": # report firmware configuration
                    pass # returns all the low level config values, like min/max axis values, maxspeed etc.

                case "M400": # wait for current moves to finish
                    pass # we always do this right away.. 

                case "M408": # report json-style response [of low level status]
                    pass
                case "M409": # query object model [report low level status]
                    pass

                case "M410": # quick stop [emergency stop]
                    pass

                case "M413": # power loss recovery controls
                    pass # stores current print job on sdcard, if enabled.. maybe just name and line of current execution?

                case "M416": # power loss
                    pass # reports that power has been lost (e.g. with power backup system to prepare for shutdown)

                case "M420": # leveling on/off/fade
                    pass # control autofocus enable/disable including refinement delta (acceptable error)

                case "M425": # backlash correction [control]
                    pass 

                case "M450": # report printer mode
                    pass # e.g. cnc | fff [filament] | laser .. we could return microscope?

                case "M500": # save current parameters to non-volatile storage
                    pass # store low level parameters somewhere
                case "M501": # load parameters from non-volatile storage
                    pass
                case "M502": # restore default settings
                    pass
                case "M503": # report current settings
                    pass
                case "M504": # validate eeprom [settings]
                    pass

                case "M505": # set configuration file folder
                    pass

                case "M532": # report print progress
                    pass # returns percent progress and layer number.. we could report other values instead

                case "M544": # gcode parser options
                    pass # can e.g. list supported gcode commands

                case "M559": # upload configuration file
                    pass # e.g. with M28..M29 (start/stop writing commands to sdcard)

                case "M593": # define or remove a tool
                    pass # perfect use case for subprograms, which could be in standard location on sdcard
                
                case "M564": # limit axes [to homed and validated volume]
                    pass

                case "M568": # tool settings
                    pass # change tool settings.. like exposure time etc.

                case "M569": # stepper driver control
                    pass # set low level motor controls

                case "M569.1": # stepper driver closed loop configuration
                    pass

                case "M574": # set endstop configuration
                    pass

                case "M575": # set serial comms parameters
                    pass # .. like baud rate

                case "M581": # configure external trigger
                    pass

                case "M590": # report current tool type and index
                    pass 

                case "M594": # enter/leave height following mode
                    pass # e.g. nikon has this

                case "M599": # define keepout zone
                    pass # i cant believe i have just slowly rediscovered things klipper can already do

                case "M905": # set local date and time
                    pass

                case "M906": # set motor currents
                    pass
                case "M909": # set microstepping
                    pass

                # M911 set tmc2130 holding currents

                case "M928": # start sd logging
                    pass
                case "M929": # start/stop event logging to sd card
                    pass
                
                case "M951": # set height following mode parameters
                    pass

                case "M955": # configure accelemerator
                    pass

                case "M997": # perform in application firmware update
                    pass
                ...
                case other:
                    error_internal(f"line {linenum}: invalid command {other}")

        return BasicSuccessResponse(msg=result_msg)
